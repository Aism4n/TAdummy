#define TA_MASQUERADE_IMMUNE(H) (H.get_vampire_generation() == GENERATION_METHUSELAH)

/datum/component/TA_vampire_disguise
...
/datum/component/TA_vampire_disguise/Initialize(upkeep = 0, min_blood = 50)
	if(!ishuman(parent))
...
/datum/component/TA_vampire_disguise/proc/cache_original_appearance(mob/living/carbon/human/H)
...
/datum/component/TA_vampire_disguise/proc/handle_disguise_upkeep(mob/living/carbon/human/source)
	SIGNAL_HANDLER
	...
	if(source in GLOB.coven_breakers_list && !TA_MASQUERADE_IMMUNE(source))
...
/datum/component/TA_vampire_disguise/proc/apply_disguise(mob/living/carbon/human/H)
	if(disguised)
		return FALSE

	// MASQUERADE LOCK CHECK
	if(H in GLOB.coven_breakers_list && !TA_MASQUERADE_IMMUNE(H))
...
/datum/component/TA_vampire_disguise/proc/remove_disguise(mob/living/carbon/human/H)
	if(!disguised)
		return FALSE
...
/datum/component/TA_vampire_disguise/proc/force_undisguise(mob/living/carbon/human/H)
	if(!disguised || TA_MASQUERADE_IMMUNE(H))
		return FALSE
...
/datum/component/TA_vampire_disguise/proc/disguise_status()
	return disguised

/datum/component/TA_vampire_disguise/nosferatu
	var/original_ear_accessory_type
	var/original_ear_accessory_colors

/datum/component/TA_vampire_disguise/nosferatu/cache_original_appearance(mob/living/carbon/human/H)
	var/obj/item/organ/ears/ears = H.getorganslot(ORGAN_SLOT_EARS)
	original_ear_accessory_type = ears?.accessory_type
	original_ear_accessory_colors = ears?.accessory_colors
	return ..()

/datum/component/TA_vampire_disguise/nosferatu/apply_disguise(mob/living/carbon/human/H)
	var/obj/item/organ/ears/ears = H.getorganslot(ORGAN_SLOT_EARS)
	ears?.set_accessory_type(original_ear_accessory_type)
	ears?.accessory_colors = original_ear_accessory_colors
	return ..()

#undef TA_MASQUERADE_IMMUNE